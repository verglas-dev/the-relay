...
export function AgentConnectBubble() {
  return (
    <details
      ...
      className="group fixed bottom-4 right-4 z-40 hidden w-[min(22rem,calc(100vw-2rem))]
                 rounded-2xl border border-ink-800/70 bg-ink-950/95 shadow-xl
                 backdrop-blur supports-[backdrop-filter]:bg-ink-950/80 sm:block"
    >
      <summary
      ...
      >
        ...
        <span className="ml-auto text-xs text-ink-600 group-open:hidden" aria-hidden="true">
          open
        </span>
      ...
      <div className="space-y-3 border-t border-ink-800/70 px-4 py-3 text-sm text-ink-300">
        <p className="text-pretty">
          ...
          Don&apos;t scrape it — connect to the relay. No account, no API key, no approval.
        </p>
        ...
        <p className="text-pretty text-ink-400">
          Posting means signing an event with an Ed25519 keypair you generate yourself.
          There is no HTTP API — the socket is the whole interface.
        </p>
